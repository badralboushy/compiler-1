...
public :
	Type_specifier* type;
	Id* id;
	Param_decl(int, int , Type_specifier* , Id*); 
...
{
public:
	virtual void visit(Id_variable* n)=0;
...
{
public :
	virtual void visit(Id_variable* n);
...
public:
	...
	Scope* current;
	Symbol_table();
	bool AddSymbol(Id* id, int kind, int type , Param_decl_list* = NULL);
	void OpenScope();
	void CloseScope();
	Symbol* CheckSymbol(Id*, int kind , Expr_list* = NULL);
};
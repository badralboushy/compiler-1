...
Param_decl::Param_decl(int line, int column , Type_specifier* t , Id* id) :Node(line, column) {
	this->type = t;
	this->id = id;
}
Normal_param_decl::Normal_param_decl(int line, int column,Type_specifier* tb, Id* id): Param_decl(line, column , tb , id) {
	
	if (id != NULL) id->father = this;
	this->type->father = this; 
}
Array_param_decl::Array_param_decl(int line, int column, Type_specifier* tb, Id* id) : Param_decl(line, column ,tb ,id) {
	this->type = tb; 
	this->id = id;
	if (id != NULL) id->father = this;
	this->type->father = this; 
...
void
PrintVisitor::visit(Id_variable* n)
{
	cout << "declaring id variable " << n->id->value<< endl;
	n->id->accept(this); 
...
///////////////////////////// visit function in TypeVisitor class ////////////////
...
void
TypeVisitor::visit(Class_decl* n)
{
	cout  <<n->accmod<< " class declaration, class name is " << n->id->value << endl;
...
void
TypeVisitor::visit(Smallc_program* n)
{
	cout << "this is function or class member declaration" << endl;
...
void
TypeVisitor::visit(Func_program* n)
{
	cout << " this is func_program declaration "<< endl;
	n->func->accept(this);
...
void
TypeVisitor::visit(Var_program* n)
{
	cout << "this is variable declaration" << endl;
	n->var_decl->accept(this);
...
void
TypeVisitor::visit(Class_decl_program* n)
{
	cout << "this is inner class declaration " << endl;
	n->class_decl->accept(this);
...
void
TypeVisitor::visit(Func* n)
{
	cout << "function declaration " << endl;
};

void
TypeVisitor::visit(Array_func* n)
{
	cout << "declaring function returns array " << endl;
	n->compound_stmt->accept(this);
	n->id->accept(this);
...
void
TypeVisitor::visit(Normal_func* n)
{
	cout << "declaring function with " << n->type->type << " return type and name " << n->id->value<< endl;
	n->id->accept(this);
	n->param_decl_list->accept(this);
	n->compound_stmt->accept(this);
...
void
TypeVisitor::visit(Param_decl* n)
{
//	cout << "declaring Param_decl " << endl;
...
void
TypeVisitor::visit(Array_param_decl* n)
{
	cout << "declaring Array_param_decl with type " << n->type->type << endl;
	n->id->accept(this);
...
void
TypeVisitor::visit(Normal_param_decl* n)
{
	cout << "declaring Normal_param_decl with type " << n->type->type << endl;
	n->id->accept(this);
...
void
TypeVisitor::visit(Param_decl_list* n)
{
	cout << "declaring Param_decl_list with size " << n->list->size() << endl;
...
void
TypeVisitor::visit(Compound_stmt* n)
{
	cout << "declaring Compound_stmt" << endl;
	n->compound_stmts->accept(this);
...
void
TypeVisitor::visit(Compound_stmts* n)
{
	cout << "declaring Compound_stmts with size of " << n->stmt_list->size() << endl;
	cout << "and variable_stmts number is " << n->var_decl_list->size() << endl;
...
void
TypeVisitor::visit(Var_decl_list* n)
{
	cout << "declaring Var_decl_list with size of " << n->list->size() << " of variables" << endl;
...
void
TypeVisitor::visit(Variable_id* n)
{
	cout << "declaring Variable_id" << endl;
...
void
TypeVisitor::visit(Variable_assignment* n)
{
	cout << "declaring Variable_assignment" << endl;
	n->id->accept(this);
	n->expr->accept(this);
	if (n->id->symbol->type != n->expr->type) {
		cout << "type mismatched at line " << n->line << " and column " << n->column << endl; 
	}

};

void
TypeVisitor::visit(Array_variable_assignment* n)
{
	cout << "declaring Array_variable_assignment" << endl;
	n->id->accept(this);
	n->primary_list->accept(this);
...
void
TypeVisitor::visit(Array_variable_id* n)
{
	cout << "declaring Array_variable_id" << endl;
	n->id->accept(this);
...
void
TypeVisitor::visit(Stmt_compound* n)
{
	cout << "declaring Stmt_compound" << endl;
	n->compound_stmt->accept(this);
...
void
TypeVisitor::visit(Stmt_while* n)
{
	cout << "declaring Stmt_while" << endl;
	n->while_stmt->accept(this);
...
void
TypeVisitor::visit(Stmt_assignstmt* n)
{
	cout << "declaring Stmt_assignstmt" << endl;
	n->assignment->accept(this);
...
void
TypeVisitor::visit(If_cond_stmt* n)
{
	cout << "declaring If_cond_stmt" << endl;
	n->expr->accept(this);
	n->ifstmt->accept(this);
	if (n->expr->type != 5)
		cout << "can't convert to boolean" << endl;


};

void
TypeVisitor::visit(Else_cond_stmt* n)
{
	cout << "declaring Else_cond_stmt" << endl;
	n->expr->accept(this);
	n->ifstmt->accept(this);
	n->elsestmt->accept(this);
	if (n->expr->type != 5)
		cout << "can't convert to boolean at line " << n->expr->line << ":at column " << n->expr->column << endl; 
...
void
TypeVisitor::visit(While_stmt* n)
{
	cout << "declaring While_stmt" << endl;
	n->expr->accept(this);
	n->stmt->accept(this);
	if (n->expr->type !=5 )
		cout << "can't convert between expr type to bool at line " << n->expr->line << ": at column " << n->expr->column << endl;


};

void
TypeVisitor::visit(Assign_stmt* n)
{
	
	cout << "declaring Assign_stmt" << endl;
	n->expr->accept(this);
	n->id->accept(this);
	if (n->expr->type != n->id->symbol->type)
	{
		cout << "can't convert between left type and right type" << endl;
	}
...
void
TypeVisitor::visit(Id_variable* n)
{
	//cout << "declaring variable id " << n->id->value << endl;
	n->id->accept(this);
...
void
TypeVisitor::visit(Primary_num* n)
{
	cout << "declaring Primary_num with value of " << n->value << endl;
	n->type = 1;
};
...
void
TypeVisitor::visit(Primary_float* n)
{
	cout << "declaring Primary_float with value of " << n->value << endl;
	n->type = 2;
...
void
TypeVisitor::visit(Primary_char* n)
{
	cout << "declaring Primary_char with value of " << n->value << endl;
	n->type = 3;
...
void
TypeVisitor::visit(Primary_string* n)
{
	cout << "declaring Primary_string with value of " << n->value << endl;
	n->type = 4;
...
void
TypeVisitor::visit(Primary_id* n)
{
	cout << "declaring Primary_id " << endl;
	n->id->accept(this);
...
void
TypeVisitor::visit(Primary_false* n)
{
	cout << "declaring Primary_false" << endl;
	n->type = 5;
...
void
TypeVisitor::visit(Primary_true* n)
{
	cout << "declaring Primary_true" << endl;
	n->type = 5;
...
void
TypeVisitor::visit(Primary_null* n)
{
	cout << "declaring Primary_null" << endl;
	n->type = 0;
...
void
TypeVisitor::visit(TripleCondition* n)
{
	cout << "declaring TripleCondition" << endl;
	n->expr1->accept(this);
	n->expr2->accept(this);
...
void
TypeVisitor::visit(Exp_primary* n)
{
	cout << "declaring Exp_primary" << endl;
	n->primary->accept(this);
	n->type = n->primary->type;
};

void
TypeVisitor::visit(Exp_triplecondition* n)
{
	cout << "declaring Exp_triplecondition" << endl;
	n->triple_condition->accept(this);
...
void TypeVisitor::visit(Operator_expr* n)
{
	cout << "declaring Operator_expr" << endl;
	n->expr_left->accept(this);
	n->expr_right->accept(this);
	n->op->accept(this);
	int left_type = n->expr_left->type;
	int right_type = n->expr_right->type;
	if (n->op->op_code == 1) {
		if (left_type == 4 || right_type == 4) {
			n->type = 4;
		}
		else if (left_type != 5 && right_type != 5 && left_type != 0 && right_type != 0) {
			...
			else if (left_type == 1 && right_type == 2 || left_type == 2 && right_type == 1) {
				n->type = 2;
			}
			else if (left_type == 1 && right_type == 3 || left_type == 3 && right_type == 1) {
				n->type = 1;
			}
			else if (left_type == 2 && right_type == 3 || left_type == 3 && right_type == 2) {
				n->type = 2;
			}
		...
		// 4 string 
		// 5 bool
		}else if (n->op->op_code == 2 || n->op->op_code == 3 || n->op->op_code == 4) {

		
			if (left_type == 1 && right_type == 1)n->type = 1;

			else if (left_type == 2 && right_type == 2) n->type = 2;

			else if ((left_type == 2 && right_type == 1) || (right_type == 2 && left_type == 1))
				n->type = 2; 

			else
			{
				cout << "undefined operator for these types" << endl; 
			}


		}
		else if (n->op->op_code == 5 && n->op->op_code == 6) {
		if (left_type == right_type == 1 || left_type== right_type == 2 || left_type ==right_type ==3 || left_type== right_type ==4 )
			n->type = 5;
		else if (left_type == 1 && right_type == 2 || left_type == 2 && right_type == 1)
		{	
			n->type = 5;
		...
		else if (n->op->op_code == 7 && n->op->op_code == 9)
		{	
				
				if (left_type == 5 && right_type == 5)n->type = 5;
				
				else
					cout << "type mismatch for this operator " << endl;
		}
		
		else if (n->op->op_code == 8)
		{
			if (right_type == left_type)n->type = 5;
			else if (right_type == 1 && left_type == 2 || right_type == 2 && left_type == 1)n->type = 5; 
			else
			{
				cout << "type mismatch for this operator " << endl; 
			}
...
void
TypeVisitor::visit(Parentheses_expr* n)
{
	cout << "declaring Parentheses_expr" << endl;
	n->expr->accept(this);
	n->type = n->expr->type; 
...
void
TypeVisitor::visit(Minus_expr* n)
{
	cout << "declaring Minus_expr" << endl;
	n->expr->accept(this);
	n->type = n->expr->type; 
};
// make type checking here not in the parser 
void
TypeVisitor::visit(Func_call_expr* n)
{
	cout << "declaring Func_call_expr" << endl;
	n->id->accept(this);
	n->list->accept(this);
...
void
TypeVisitor::visit(Array_expr* n)
{
	cout << "declaring Array_expr" << endl;
	n->expr->accept(this);
	n->id->accept(this);
...
void
TypeVisitor::visit(Primary_list* n)
{
	cout << "declaring Primary_list with size of " << n->list->size() << " primaries" << endl;
...
void
TypeVisitor::visit(Type_specifier* n)
{
	cout << "declaring Type_specifier with code " << n->type << endl;
...
bool Symbol_table::AddSymbol(Id* id, int kind, int type , Param_decl_list* p ) {
	//cout << "Hello samara" << endl; 
	Symbol* s = new Symbol(id->value, kind, type);
	string key;
	if (kind == 1) {
		key = "v" + id->value; // the symbol is a variable
	}
	else {
		key = "f" + id->value; //the symbol is  a function name;

		if (p->list->size() > 0) {
			for (int i = 0; i < p->list->size(); i++) {
				key += p->list->at(i)->type->type;
			}
		}
	}
	...
	if (temp == NULL) {
		...
		cout << "redifined variable: line " << id->line << ", column " << id->column << endl;
		return false;
	}
}

Symbol* Symbol_table::CheckSymbol(Id* id, int kind , Expr_list * er) {
	string key;
	if (kind == 1) {
		key = "v" + id->value;
	}
		...
		if (er->list->size() > 0) 
		for(int i=0 ; i<er->list->size() ; i++)
		key += er->list->at(i)->type;
	}

	Symbol* s = this->current->hashtab->GetMember(key);
	try {
		Symbol* s2 = this->scopes->at(this->scopes->size() - 2)->hashtab->GetMember(key);
		if(s == NULL && s2 == NULL)
			cout << "undeclared variable: line " << id->line << ", column " << id->column << endl;
		    return NULL;
	}
	catch(exception e){
		if (s != NULL) {
...
void Symbol_table::OpenScope() {
	this->scopes->push_back(new Scope());
	this->current = scopes->at(this->scopes->size() - 1);
 }

...
class_decl: ACCMOD CLASS ID { sym_tab->OpenScope(); } '{'smallc_program'}' { sym_tab->CloseScope(); }
					{
						$$ = new Class_decl(getLine(),getColumn(),$1,$3,$6 );
						root = $$ 

					
					}
			|CLASS ID { sym_tab->OpenScope(); } '{' smallc_program '}' { sym_tab->CloseScope(); } 
					{
					$$ = new Class_decl(getLine(), getColumn(),"private" ,$2, $5) ; 
					root = $$ ; 
					}
;
...
func: type_specifier ID { cout<< "badr " <<endl; sym_tab->OpenScope(); }  '(' param_decl_list ')'  compound_stmt { sym_tab->CloseScope(); } 
			{
			$$= new Normal_func(getLine(),getColumn(),$1,$2,$5,$7);
			sym_tab->AddSymbol($2 , 2, $1->type , $5);
			}
	|type_specifier '['']' ID { sym_tab->OpenScope(); }  '(' param_decl_list ')'  compound_stmt { sym_tab->CloseScope(); }
			{
			$$ = new Array_func(getLine(),getColumn(), $1, $4 ,$7 , $9);
			sym_tab->AddSymbol($4 , 2 , $1->type);
			}
;
type_specifier: INT
					{
...
compound_stmt:'{' { sym_tab->OpenScope(); }  compound_stmts  { sym_tab->CloseScope(); }  '}'
					{
						$$ = new Compound_stmt(getLine(),getColumn(),$3);
					}
;
compound_stmts:  
					{
...
variable_id:	ID
					{
						...
						sym_tab->AddSymbol($1 ,1, current_type);
